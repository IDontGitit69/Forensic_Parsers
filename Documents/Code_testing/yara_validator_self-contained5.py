...
def count_braces_outside_strings(text):
    """
    Count braces that are NOT inside string literals or comments.
    
    Returns: (open_braces_count, close_braces_count)
    """
    in_string = False
    in_comment = False
...
class YaraValidator:
    ...
    def _attempt_repair(self, source, imports=''):
        ...
        # Fix missing braces - use smart counting
        open_braces, close_braces = count_braces_outside_strings(repaired)
        
        if open_braces != close_braces:
            if open_braces > close_braces:
                repaired += '\n}'
                repairs.append("Added missing closing brace")
            # Don't try to fix if there are MORE closing braces than opening
...
if __name__ == '__main__':
    main(), line):
            ...
            if not re.match(r'^\s*import\s+"[^"]+"\s*
...
if __name__ == '__main__':
    ...
    for match in matches:
        ...
        # Verify it's a complete rule by counting braces OUTSIDE strings and comments
        open_braces, close_braces = count_braces_outside_strings(rule_source)
        
        if open_braces == close_braces:
            rules.append(rule_source)
        elif open_braces > close_braces:
            # Only add braces if genuinely missing (not in strings)
            missing_braces = open_braces - close_braces
            print(f"  ⚠️  Rule appears to be missing {missing_braces} closing brace(s), attempting to fix...", file=sys.stderr)
            rule_source += '\n' + '}' * missing_braces
            rules.append(rule_source)
        else:
            # More closing than opening - likely malformed, but include it anyway
            print(f"  ⚠️  Rule has {close_braces - open_braces} extra closing brace(s)", file=sys.stderr)
            rules.append(rule_source)
